/*
 * Trong Java, các ký tự hậu tố như `L`, `F`, `D` sau các số nguyên và số thực được dùng để chỉ định rõ ràng kiểu dữ liệu của các số đó. Dưới đây là các tình huống cụ thể khi cần và không cần sử dụng các ký tự này:
...
3. **D (Double)**:
   - Thực ra, `D` không nhất thiết phải dùng vì Java mặc định sẽ hiểu số thực là kiểu `double`. Tuy nhiên, bạn có thể dùng `D` để rõ ràng hơn. Ví dụ:
     ```java
     double num = 3.14D;
     ```
...
- Có thể dùng `D` khi muốn rõ ràng chỉ định số thực là kiểu `double`, nhưng không bắt buộc vì Java mặc định hiểu số thực là kiểu `double`.
 */